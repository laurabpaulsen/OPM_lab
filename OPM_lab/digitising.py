...
class DigitisationPlotter:
    ...
    def update_message_box(self, category, label, message):
        """Updates the message box with the current sensor information."""
        self.ax_text.clear()
        self.ax_text.axis('off')  # Turn off axis lines
        self.ax_text.text(0.5, 0.4, f'{category}: {label}', va='center', ha='center', fontsize=14)
        self.ax_text.text(0.5, 0.8, message, va='center', ha='center', fontsize=14, color='red')
...
class Digitiser:
    def __init__(self, connector: FastrakConnector, plotter: DigitisationPlotter = None):
        """
        Args:
    ...
    def update_digitised_data(self, category, label, position):
        """
        ...
        Args:
            ...
            ax_3d (Axes3DSubplot): The 3D axes object to update the plot.
        """
    ...
    def digitise_continuous(self, category, n_points, ax_3d, ax_text):
        self.plotter.update_message_box(ax_text, label=category, category=category,
        ...
        while idx < n_points:
            ...
            self.update_digitised_data(category=category, label=category, position=position, ax_3d=ax_3d)
            idx += 1  # Move to the next point
            self.plotter.refresh_plot()

        play_sound("done")

    def digitise_single(self, category, labels, limit = 30):
        """
        ...
        self.plotter.update_message_box(label=labels[idx], category=category, message="Ready for digitization...")
        self.plotter.refresh_plot()
        ...
        while idx < len(labels):
            ...
            if idx <= len(labels)-1:
                self.plotter.update_message_box(label=labels[idx], category=category, message="Now digitising:") 
            
            if cont:
                self.update_digitised_data(category=category, label=labels[idx - 1], position=position)
