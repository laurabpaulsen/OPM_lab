from .sensor_locations import OPMSensorLayout
import pandas as pd
...
def add_dig_montage(mne_object, df:pd.DataFrame):
    ...
    fiducials = {label: df.loc[df["label"] == label, ["x", "y", "z"]].values.squeeze() / 100
                 for label in required_labels}
    head_points = df[df["label"] == "head"].loc[:, ["x", "y", "z"]].values / 100

    dig_montage = make_dig_montage(nasion=fiducials["nasion"], lpa=fiducials["lpa"],
                                   rpa=fiducials["rpa"], hsp=head_points, coord_frame="head")
...
def add_sensor_layout(mne_object, sensor_layout:OPMSensorLayout):
    ...
    for pos, ori, label in zip(sensor_layout.chan_pos, sensor_layout.chan_ori, sensor_layout.label):
        idx = next((idx for idx, ch in enumerate(mne_object.info["chs"]) if ch["ch_name"] == label), None)
        if idx is None:
...
def add_device_to_head(mne_object, digitised_points):
    ...
    trans = _quat_to_affine(_fit_matched_points(np.array(sensors_device), sensors_head)[0])
